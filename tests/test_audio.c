    ...
    for (int i = 0; i < 24; i++)
    {
      ...
      for (int button_idx = 0; button_idx < keypad_get_button_count(); button_idx++) {
        if (keypad_is_button_pressed(button_idx)) {
          ...
          if (event == INPUT_KEYPAD_1) {
                mixer_init(&chord);
                mixer_add(&chord, 261.63f);
                play_chord(&chord, 200);
          }
          if (event == INPUT_KEYPAD_2) {
                mixer_init(&chord);
                mixer_add(&chord, 293.66f);
                play_chord(&chord, 200);
          }
            if (event == INPUT_KEYPAD_3) {
                    mixer_init(&chord);
                    mixer_add(&chord, 329.63f);
                    play_chord(&chord, 200);
            }
            if (event == INPUT_KEYPAD_4) {
                    mixer_init(&chord);
                    mixer_add(&chord, 349.23f);
                    play_chord(&chord, 200);
            }
            if (event == INPUT_KEYPAD_5) {
                    mixer_init(&chord);
                    mixer_add(&chord, 392.00f);
                    play_chord(&chord, 200);
            }
            if (event == INPUT_KEYPAD_6) {
                    mixer_init(&chord);
                    mixer_add(&chord, 440.00f);
                    play_chord(&chord, 200);
            }
            if (event == INPUT_KEYPAD_7) {
                    mixer_init(&chord);
                    mixer_add(&chord, 493.88f);
                    play_chord(&chord, 200);
            }
            if (event == INPUT_KEYPAD_8) {
                    mixer_init(&chord);
                    mixer_add(&chord, 523.25f);
                    play_chord(&chord, 200);
            }
            if (event == INPUT_KEYPAD_9) {
                    mixer_init(&chord);
                    mixer_add(&chord, 261.63f);
                    mixer_add(&chord, 329.63f);
                    mixer_add(&chord, 392.00f);
