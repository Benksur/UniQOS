...
int main(void)
{
  ...
  while (1)
  {
    memset(response, 0, sizeof(response));
    // res = HAL_UART_Transmit(&huart1, (uint8_t *)"AT\r\n", 20, HAL_MAX_DELAY);
    res = HAL_UART_Receive(&MODEM_UART_HANDLE, response, 100, 10);
    //  printf("%s, %d", response, res);
    ret = modem_send_command("AT", response, sizeof(response), TIMEOUT_1S);
    ret = modem_send_command("AT+CPIN?", response, sizeof(response), TIMEOUT_1S);
    printf("%s, %d", response, ret);
