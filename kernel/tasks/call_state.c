...
{
    if (!ctx->display_ctx) return;
    
    DisplayNotificationData display_data = {
        .call_state = (uint8_t)ctx->current_state,
    ...
    switch (msg->cmd)
    {
        case CALL_CMD_INCOMING_CALL:
            if (ctx->current_state == CALL_STATE_IDLE)
            {
                ...
                if (msg->data)
                {
                    DisplayNotificationData *notif_data = (DisplayNotificationData *)msg->data;
                    strncpy(ctx->caller_id, notif_data->caller_id, sizeof(ctx->caller_id) - 1);
...
CallStateContext *CallState_Init(DisplayTaskContext *display_ctx)
{
    ...
    call_ctx.task_handle = thread_id;
    return &call_ctx;
}
