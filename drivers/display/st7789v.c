...
static void ST7789_WriteCommand(uint8_t cmd)
{
  ...
  uint16_t cmd2 = cmd;
	// ST7789_DC_Clr();
	HAL_SPI_Transmit(&ST7789_SPI_PORT, (uint8_t*)&cmd2, sizeof(cmd2), HAL_MAX_DELAY);
	ST7789_UnSelect();
}
...
static void ST7789_WriteSmallData(uint8_t data)
{
  ...
  uint16_t data2 = data|(1<<8);
	HAL_SPI_Transmit(&ST7789_SPI_PORT, (uint8_t*)&data2, sizeof(data2), HAL_MAX_DELAY);
	ST7789_UnSelect();
}
...
static void ST7789_WriteData(uint8_t *buff, size_t buff_size)
{
  ...
  for(int i = 0; i < buff_size; i++){
    ST7789_WriteSmallData(buff[i]);
  }

	// while (buff_size > 0) {
	// 	uint16_t chunk_size = 1;
  //   ST7789_WriteSmallData(*buff);
	// 	buff += chunk_size;
	// 	buff_size -= 1;
	// }
